...
with st.sidebar:
    ...
    if not st.session_state["is_admin"]:
        with st.expander("🔑 관리자 메뉴 열기"):
            ...
            if pwd:
                if pwd == st.secrets.get("ADMIN_PASSWORD", "설정오류"):
                    st.session_state["is_admin"] = True
                    st.rerun()
                else:
                    st.error("비밀번호 불일치")
    else:
        st.success("👑 관리자 모드 켜짐")
...
app_mode = st.sidebar.radio(
    ...
    index=None,  # 핵심: 처음에 아무것도 선택하지 않음
)


def render_home():
    st.title("🏡 두두타 데이터베이스 & 효율 트래커")
    st.markdown("---")

    st.markdown(
        """
    환영합니다! 좌측 사이드바에서 원하시는 **실험 데이터 트래커**나 **요리 효율 계산기**를 선택해 주세요.
    
    ### ⚠️ 데이터 측정 환경 안내
    본 사이트에 기록된 모든 데이터는 **관련 취미(원예, 요리, 낚시 등) 레벨 10(Max)** 상태에서 측정되었습니다.
    
    (※ 취미 레벨이 실제 수확량이나 결과물 성급에 미치는 정확한 영향도는 아직 명확히 밝혀지지 않았으므로 참고용으로만 활용해 주세요.)
    """
    )
    st.markdown("---")
...
def render_gardening():
    ...
    if st.session_state["is_admin"]:
        tabs = st.tabs(
            ["📊 데이터 분석 및 필터링", "📝 데이터 입력", "🗑️ 데이터 관리(삭제)"]
        )
        tab_analysis, tab_input, tab_manage = tabs[0], tabs[1], tabs[2]
    else:
        tabs = st.tabs(["📊 데이터 분석 및 필터링"])
        tab_analysis = tabs[0]

    with tab_analysis:
        st.header("원예 실험 결과 분석")
        df = load_query_data("SELECT * FROM experiments")

        if df.empty:
            st.info("아직 입력된 데이터가 없습니다.")
        else:
            with st.expander("🌱 원예 조건 필터링", expanded=True):
                ...
                with f_col2:
                    f_water = st.multiselect(
                        "물 별 개수", options=[1, 2, 3, 4, 5], default=[5]
                    )
                    f_weed_map = st.selectbox(
                        "잡초 생성 비트맵 (4-bit)",
                        ["전체"] + list(df["weed_bitmap"].unique()),
                    )
                with f_col3:
                    f_weed_rm = st.selectbox(
                        "잡초 제거", ["전체", "제거함(O)", "방치함(X)"], index=2
                    )
                    f_weed_rm_after = st.selectbox(
    ...
    if st.session_state["is_admin"]:
        with tab_input:
            ...
            with col1:
                ...
                weed_removed_after = st.checkbox(
                    "방치 후 제거 여부",
                    key="g_weed_rm_after",
                    disabled=not weed_removed,
                )
                unattended_time = st.number_input(
                    "성장 후 방치 시간 (30분 단위 정수)",
                    min_value=0,
                    step=1,
                    key="g_time",
                )
            ...
            with col2:
                ...
                star_1 = st.number_input(
                    "1성 아이템 개수", min_value=0, step=1, key="g_s1"
                )
                star_2 = st.number_input(
                    "2성 아이템 개수", min_value=0, step=1, key="g_s2"
                )
                star_3 = st.number_input(
                    "3성 아이템 개수", min_value=0, step=1, key="g_s3"
                )
                star_4 = st.number_input(
                    "4성 아이템 개수", min_value=0, step=1, key="g_s4"
                )
                star_5 = st.number_input(
                    "5성 아이템 개수", min_value=0, step=1, key="g_s5"
                )
        ...
        with tab_manage:
            ...
            df_all = pd.read_sql_query(
                "SELECT * FROM experiments ORDER BY id DESC", conn
            )
            conn.close()
...
def render_cooking():
    ...
    if st.session_state["is_admin"]:
        ...
        tabs = st.tabs(["📊 요리 데이터 분석 및 필터링"])
        tab_analysis = tabs[0]

    recipe_df = load_query_data(
    ...
    with tab_analysis:
        ...
        if df_cook.empty:
            ...
            with st.expander("🍳 요리 조건 필터링", expanded=True):
                col_f1, col_f2 = st.columns(2)
                with col_f1:
                    f_ingredient_tier = st.selectbox(
                        "재료 성급 카테고리 (접미사 기준)",
                        [
                            "1/2성 재료 (접미사 없음)",
                            "3성 재료 (_3)",
                ...
                with col_f2:
                    f_recipe = st.selectbox(
                        "레시피 이름",
                        ["전체"] + list(filtered_cook["recipe_name"].unique()),
                    )
...
def render_apple():
    ...
    if st.session_state["is_admin"]:
        tabs = st.tabs(
            ["📊 채집 효율 분석", "📝 채집 데이터 입력", "🗑️ 채집 데이터 관리(삭제)"]
        )
        tab_analysis, tab_input, tab_manage = tabs[0], tabs[1], tabs[2]
    else:
        tabs = st.tabs(["📊 채집 효율 분석"])
        tab_analysis = tabs[0]
    ...
    with tab_analysis:
        ...
        df_foraging = load_query_data("SELECT * FROM foraging_experiments")
...
def render_mushroom():
    ...
    for idx, (m_eng, m_kor) in enumerate(mushroom_names.items()):
        with tabs[idx]:
            st.header(f"[{m_kor}] 채집 데이터 분석")
            df_m = load_query_data(
            ...
            if st.session_state["is_admin"]:
                ...
                if not df_m.empty:
                    with st.expander(f"🗑️ {m_kor} 데이터 삭제"):
                        conn = get_connection()
                        df_m_del = pd.read_sql_query(
                            "SELECT * FROM mushroom_experiments WHERE mushroom_type = %s ORDER BY id DESC",
                            conn,
                            params=(m_eng,),
                        )
                        conn.close()
                        st.dataframe(df_m_del, use_container_width=True)
                        ...
                        if st.button("해당 ID 데이터 삭제", key=f"btn_del_{m_eng}"):
                            ...
                            c = conn.cursor()
                            c.execute(
                                "DELETE FROM mushroom_experiments WHERE id = %s",
                                (del_id,),
                            )
                            conn.commit()
                            conn.close()
...
def render_fishing():
    ...
    if st.session_state["is_admin"]:
        tabs = st.tabs(
            ["📊 낚시 효율 분석", "📝 낚시 데이터 입력", "🗑️ 낚시 데이터 관리(삭제)"]
        )
        tab_analysis, tab_input, tab_manage = tabs[0], tabs[1], tabs[2]
    else:
        tabs = st.tabs(["📊 낚시 효율 분석"])
        tab_analysis = tabs[0]
    ...
    with tab_analysis:
        st.header("낚시 효율 분석")
        df_fish = load_query_data("SELECT * FROM fishing_experiments")
    ...
    if st.session_state["is_admin"]:
        with tab_input:
            ...
            with col1:
                ...
                f_weather = st.selectbox(
                    "☁️ 날씨", weather_options, index=auto_weather_idx
                )
        ...
        with tab_manage:
            ...
            conn = get_connection()
            df_fish_all = pd.read_sql_query(
                "SELECT * FROM fishing_experiments ORDER BY id DESC", conn
            )
            conn.close()
            if not df_fish_all.empty:
                st.dataframe(df_fish_all, use_container_width=True)
                del_id = st.number_input(
                ...
                if st.button("데이터 삭제"):
                    ...
                    c = conn.cursor()
                    c.execute(
                        "DELETE FROM fishing_experiments WHERE id = %s", (del_id,)
                    )
                    conn.commit()
                    conn.close()
                    st.rerun()
...
def render_shop():
    ...
    if st.session_state["is_admin"]:
        ...
        tabs = st.tabs(["📊 상점 단가표 및 할인 확률"])
        tab_analysis = tabs[0]

    with tab_analysis:
        st.header("상점 단가표 및 할인 확률")
        df_analysis = load_query_data(
            "SELECT d.ingredient_name, COUNT(d.id) as total_days, SUM(CASE WHEN d.is_discounted THEN 1 ELSE 0 END) as discount_days, r.discounted_price, r.base_price FROM store_discounts d JOIN store_reference r ON d.ingredient_name = r.ingredient_name GROUP BY d.ingredient_name, r.discounted_price, r.base_price"
        )
        if not df_analysis.empty:
            ...
            st.info("아직 저장된 할인 정보가 없습니다.")

    if st.session_state["is_admin"]:
        with tab_input:
            st.header("오늘의 할인 품목 입력")
            ingredients_list = load_query_data(
                "SELECT ingredient_name FROM store_reference"
        ...
        with tab_manage:
            ...
            if not df_store_all.empty:
                ...
                if st.button("데이터 삭제"):
                    ...
                    c = conn.cursor()
                    c.execute(
                        "DELETE FROM store_discounts WHERE id = %s", (delete_s_id,)
                    )
                    conn.commit()
                    conn.close()
                    st.rerun()
...
def render_efficiency():
    ...
    if st.session_state["is_admin"]:
        ...
        tab_main, tab_view, tab_add, tab_manage = tabs[0], tabs[1], tabs[2], tabs[3]
    else:
        tabs = st.tabs(["📊 정규 요리 대시보드", "📝 커스텀 요리 효율표"])
        tab_main, tab_view = tabs[0], tabs[1]
    ...
    if st.session_state["is_admin"]:
        ...
        with tab_manage:
            ...
            def delete_custom_row(table, row_id):
                ...
                conn.close()
                st.success(
                    f"ID {row_id} 삭제 완료! (적용을 위해 화면을 새로고침해주세요)"
                )
            ...
            with col_m1:
                ...
                if not df_custom.empty:
                    ...
                    r_id = st.number_input(
                        "삭제할 요리 ID", min_value=0, key="d_recipe"
                    )
